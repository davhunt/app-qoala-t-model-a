...
packages <- c("caret", "corrplot", "gbm", "plyr", "randomForest", "e1071",
              "pROC", "DMwR","dplyr","pbkrtest","car","doParallel","ROSE","repmis")
...
#inputFolder <- "/N/u/davhunt/Carbonate/Downloads/qoala/fs_output6/"
inputFolder <- paste(args[1],"/workdir/",sep="")
# Create output directory if it doesnt exist  
#outputFolder <- "/N/u/davhunt/Carbonate/Downloads/qoala/fs_output6/Output_Qoala_T/"
outputFolder <- paste(args[1],"/Output_Qoala_T/",sep="")
...
stats2Table <- read.csv(paste(args[1],"/Output_Qoala_T/FreeSurfer_Output_", dataset_name, ".csv", sep=""), header=T, row.names=1)
...
filename <- paste(outputFolder,"Figure_Rating_model_based_",dataset_name,".pdf",sep="")
ggsave(filename, plot=p, device="pdf", scale=1.5, width=30/2.54, height=20/2.54, units=c("in","cm","mm"),dpi=300,limitsize=TRUE)
